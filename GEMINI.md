...
- After running `maintaindb/aggregator.py`, it's crucial to verify the `catalog.json` file using `jq` to ensure the data is correctly aggregated and formatted. For example, to check the `hours` field for a specific adventure:
  ```
jq ".[] | select(.n == \"Your Adventure Title\") | .h" assets/data/catalog.json
  ```
...
- The user manages their virtual environment using `uv`.
- Use `uv run` to execute scripts in the environment, for example: `uv run -m maintaindb.aggregator`.
- When running tests: `uv run -m unittest discover maintaindb`.
