...
*   `aggregator.py`: Aggregates individual adventure JSON files (from `_dc/`) into a minified `catalog.json` file used by the Jekyll site's filter. This script normalizes data (codes, seasons, campaigns, tiers, hours) and uses abbreviated keys to reduce the payload size. It should be run after new or updated JSON files are created in `_dc/`.
